...
%     Generate a random graph
    criteria = 0;
    while criteria == 0
        [adjacency, ~] = waxtop(0.12,0.7,0.7,[0 10 0 10]);
    ...
    %                         disp([s,d]);
            ...
            %     Simulation type #2
                ...
                File = fullfile(cd, ['Desktop Tradeoff A',num2str(ii),...
                    '--',num2str(kk),'--',num2str(jj)]);
                save(File);

                if jj>1
                    File_Old = fullfile(cd, ['Desktop Tradeoff A',num2str(ii),...
