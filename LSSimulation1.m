function [meanTrafficDemand,average_Dep_Net_Congestion1,average_Dep_Net_Congestion2] = LSSimulation1()
...
%     Generate non-stationary traffic
    ...
    if stationary == 0
        unif_rand1 = 2*rand(N,N,1)+2;
        unif_rand2 = 2*rand(N,N,D/dT-1)+2;
    ...
    for kk = 1:length(delay)
        ...
        [average_Dep_Net_Congestion1(:,kk),~...
...
%         
        %     Simulation UnitOSPF
        [average_Dep_Net_Congestion3(:,kk),~] = UnitOSPF(iN,...
...
% plot(1:iN,200*meanTrafficDemand(1:iN),'LineWidth',1.5);hold on;
...
File = fullfile(cd, ['Data final 175-2-4']);
save(File);
