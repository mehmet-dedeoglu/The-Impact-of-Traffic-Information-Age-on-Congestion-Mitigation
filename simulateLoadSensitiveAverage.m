...
%     Generate non-stationary traffic
    ...
    if stationary == 0
        ...
        AR = cat(3,0.75*ones(N,N,1),0.75*ones(N,N,D/dT-1));
    end
...
%     Simulation type #3
%     [Dep_Net_Congestion3,Dep_Net_Uncertainty3] = simulateLoadSensitiveAverageInner(iN,...
%         capacities,ActualTraffic,PredictedTraffic,TrafficVariance3D...
%         ,TrafficVariance3D_Est,N,M);
%     average_Dep_Net_Congestion3 = average_Dep_Net_Congestion3...
%         + Dep_Net_Congestion3/sim_num;
%     worst_Dep_Net_Congestion3 = max(worst_Dep_Net_Congestion3,Dep_Net_Congestion3);
%     average_Dep_Net_Uncertainty3 = average_Dep_Net_Uncertainty3...
%         + Dep_Net_Uncertainty3/sim_num;
%     disp(['sim#3 ',num2str(i)]);

    File = fullfile(cd, ['Laptop Computer H',num2str(i)]);
    save(File);
    
    if i>1
        File_Old = fullfile(cd, ['Laptop Computer H',num2str(i-1),'.mat']);
        delete(File_Old);
    end
